...
def policy_evaluation(P, nS, nA, policy, gamma=0.9, tol=1e-8):
    ...
    while True:
        prev_value_function=deepcopy(value_function)
        value_function = np.zeros(nS)
        for x in range(nS):
            for y in range(nA):
                pie=policy[x][y]
                Problist=P[x][y]
                for z in range(np.shape(Problist)[0]):
                    prob=Problist[z][0]
                    nexts=Problist[z][1]
                    rew=Problist[z][2]
                    value_function[x]+=pie*prob*(rew+gamma*prev_value_function[nexts])
        if abs(max(value_function- prev_value_function)) < tol : 
            break
    ############################
    ...
    for x in range(nS):
        ...
        for y in range(nA):
            ...
            for z in range(np.shape(Problist)[0]):
                ...
                nexts=Problist[z][1]
                rew=Problist[z][2]
                val_list[y]+=prob*(rew+gamma*value_from_policy[nexts])
        new_policy[x][:]=0
        new_policy[x][np.argmax(val_list)]=1
	############################
    return new_policy
...
def policy_iteration(P, nS, nA, policy, gamma=0.9, tol=1e-8):
    ...
    Pol=policy_improvement(P, nS, nA,V,gamma)
    while Pol.all() != new_policy.all():
        new_policy=deepcopy(Pol)
        V=policy_evaluation(P,nS,nA,new_policy,gamma,tol)
        Pol=policy_improvement(P, nS, nA,V,gamma)
    V=policy_evaluation(P,nS,nA,Pol,gamma,tol)
	############################
    return Pol, V
...
def value_iteration(P, nS, nA, V, gamma=0.9, tol=1e-8):
    ...
    # YOUR IMPLEMENTATION HERE #
    while True:
        prev_value_function=deepcopy(V_new)
        value_function = np.zeros(nS)
        for x in range(nS):
            val_list=np.zeros(nA)
            for y in range(nA):
                ...
                for z in range(np.shape(Problist)[0]):
                    ...
                    nexts=Problist[z][1]
                    rew=Problist[z][2]
                    val_list[y]+=prob*(rew+gamma*prev_value_function[nexts])
            V_new[x]=max(val_list)
            policy_new[x][:]=0
            policy_new[x][np.argmax(val_list)]=1
        if abs(max(V_new - prev_value_function)) < tol :
            break
    ############################
