...
from src.randomise_action import OrnsteinUhlenbeckProcess
import torch
...
class ReinforceAgent():
    def __init__(self, state_size, action_size):
        ...
        self.ou_mu = 0.2
        self.ou_sigma = 0.5
        self.random_noise = OrnsteinUhlenbeckProcess(size=self.n_actions, theta=self.ou_theta, mu=self.ou_mu, sigma=self.ou_sigma)
    ...
    def getAction(self, state, test = False):
        # process state
        self.actor.eval()
        action = self.actor(torch.from_numpy(np.array([state]))).squeeze(0).item()
        action += (1-test)*max(self.epsilon,0)*self.random_noise.sample()
        action = np.clip(action,-1.,1.)
        state_t = torch.Tensor(np.array(state)).to(self.self.device).unsqueeze(0)
...
if __name__ == '__main__':
    ...
    for e in range(agent.load_episode + 1, agent.n_iterations):
        ...
        for t in range(agent.episode_step):
            ...
            if t >= 2500: # changed this from 500 to 1200 steps
